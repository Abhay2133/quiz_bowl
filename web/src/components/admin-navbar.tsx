...
export default function AdminNav({ backHref, path }: { backHref: string, path: { label: string, href?: string }[] }) {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (i < path.length - 1) BreadCrumbItems.push((<BreadcrumbItem key={i*2}>
      {href ? <BreadcrumbLink href={href}>{label}</BreadcrumbLink> : label}
    </BreadcrumbItem>

    ))
    if (i == path?.length - 1) BreadCrumbItems.push(
  ...
  <div className="flex p-5 gap-5 items-center">
    <div className="h-[40px] w-[40px] flex items-center justify-center border hover:bg-secondary rounded-full" onClick={() => router.back()}>
      <ArrowLeft size={20} />
    </div>
